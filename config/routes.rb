Htwk2ical::Application.routes.draw do
  ...
  scope "(:locale)", :locale => /en|de/ do
    ...
    match "calendar(/:subject_ids)" => "calendar#choose_subjects",
          :constraints => {:subject_ids => /(-?\d+)+/},
          :as          => 'calendar'

    match "calendar/courses(/:subject_ids)" => "calendar#choose_courses",
          :constraints => {:subject_ids => /(-?\d+)+/},
          :as          => 'calendar_courses'
  ...
  match "/studium_generales" => "calendar#get_studium_generales",
        :as       => 'studium_generales',
        :defaults => {:format => 'json'}
